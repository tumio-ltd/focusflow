...
export function BottomTimeline({
  ...
  return (
    <footer
      ...
      className="h-20 border-t border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md px-4 flex items-center gap-4 select-none z-20 shrink-0 transition-colors duration-200"
    >
      {/* 1. 左侧播放控制组 */}
      <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-950/70 p-1.5 rounded-xl border border-slate-200 dark:border-slate-800 shrink-0">
      ...
      <div className="flex items-center gap-2.5 overflow-x-auto flex-1 py-1 no-scrollbar">
          ...
          return (
            <div
              ...
              className={`group relative flex items-center gap-3 px-3 py-2 rounded-xl border text-xs cursor-pointer transition-all shrink-0 min-w-[180px] ${
                isDragging ? 'opacity-40 scale-95 border-dashed border-cyan-500' : ''
              } ${
                isOver ? 'ring-2 ring-cyan-500 scale-105' : ''
              } ${
                isActive
                  ? 'border-cyan-500 bg-cyan-50 dark:bg-cyan-950/30 text-cyan-900 dark:text-cyan-200 shadow-md shadow-cyan-500/10 dark:shadow-cyan-950/50 ring-1 ring-cyan-500/30 font-medium'
                  : 'border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/50 text-slate-700 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700 hover:bg-white dark:hover:bg-slate-800/40 hover:text-slate-900 dark:hover:text-slate-200'
              }`}
            >
              {/* 场景微缩标志指示 */}
              <div
                className={`w-7 h-7 rounded-lg flex items-center justify-center border text-[11px] font-mono shrink-0 transition ${
                  isActive
                    ? 'bg-cyan-500 text-white dark:text-slate-950 border-cyan-400 font-bold shadow-sm shadow-cyan-500/30'
                    : 'bg-slate-200 dark:bg-slate-800/80 text-slate-600 dark:text-slate-400 border-slate-300 dark:border-slate-700'
                }`}
              >
                ...
                {isEditing ? (
                  <div className="flex items-center gap-1">
                    <input
                      ...
                      className="bg-white dark:bg-slate-900 border border-cyan-500 rounded px-1.5 py-0.5 text-xs text-slate-900 dark:text-white focus:outline-none w-full"
                    />
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      className="p-0.5 text-cyan-600 dark:text-cyan-400 hover:text-cyan-500"
                    >
                  ...
                  <span
                    ...
                    className="truncate text-xs font-medium hover:text-cyan-600 dark:hover:text-cyan-300"
                    title="双击就地修改标题"
                  >
                    {scene.title}
                  </span>
              ...
              <div className="opacity-0 group-hover:opacity-100 flex items-center gap-0.5 transition">
                <button
                  ...
                  className="p-1 text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-300 transition"
                  title="重命名场景"
                >
        ...
        <Button
          ...
          onClick={onAddScene}
          className="border-dashed border-slate-300 dark:border-slate-700 hover:border-cyan-500/80 text-slate-600 dark:text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-300 gap-1.5 h-11 px-3.5 rounded-xl shrink-0"
        >
      ...
      {/* 4. 右侧故事板总时长 */}
      <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400 font-mono shrink-0 pl-2 border-l border-slate-200 dark:border-slate-800">
