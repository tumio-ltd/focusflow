  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-150">
      ...
      >
        ...
        {/* 分类 Tag 与搜索栏 */}
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 px-6 py-3 border-b border-slate-800 bg-slate-950/40">
          {/* 分类过滤器 */}
        ...
        <div className="p-6 overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-4 flex-1">
          {filteredTemplates.map((tpl) => (
            <div
              key={tpl.id}
              className={`group relative flex flex-col justify-between p-4 rounded-xl border bg-slate-950/60 transition-all duration-200 hover:shadow-xl ${
                tpl.featured
                  ? 'border-cyan-500/60 hover:border-cyan-400 hover:shadow-cyan-950/30'
                  : 'border-slate-800 hover:border-slate-700 hover:bg-slate-800/30'
              }`}
            >
              <div>
                {/* 封面与 Badge */}
                <div className="relative h-32 rounded-lg overflow-hidden border border-slate-800/80 mb-3.5 bg-slate-900">
                  <img
                    src={tpl.coverImage}
                    alt={tpl.title}
                  ...
                  <div className="absolute top-2.5 left-2.5 flex items-center gap-1.5">
                    <Badge variant="slate" className="backdrop-blur-md bg-slate-900/80 border-slate-700/80 text-[10px]">
                      {tpl.categoryLabel}
                    </Badge>
                    {tpl.featured && (
                ...
                {/* 标题与描述 */}
                <h4 className="text-xs font-bold text-slate-100 group-hover:text-cyan-300 transition line-clamp-1">
                  {tpl.title}
                </h4>
