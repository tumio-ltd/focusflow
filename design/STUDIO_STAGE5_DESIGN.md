...
- [ ] **Task 5.1: 纯前端单文件独立 HTML 编译器 (Standalone HTML Packager)**
  - [ ] 5.1.1 编写 `src/services/standalonePackager.ts`：实现 CSS、Player IIFE 运行时与 DSL 的内存拼装
  - [ ] 5.1.2 实现 `downloadStandaloneHtml(dsl)`：Blob 封装与浏览器 0 延迟一键触发下载
