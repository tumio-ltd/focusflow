...
- [2. 新项目制作标准化工作流 (3 步极速流)](#2-新项目制作标准化工作流-3-步极速流)
...
- [8. 控制栏配置与海量场景 (Scenes) 扩展与极限](#8-控制栏配置与海量场景-scenes-扩展与极限)
  - [8.1 在 config.json 中定制控制栏 (meta.controls)](#81-在-configjson-中定制控制栏-metacontrols)
  - [8.2 场景页码计数器设计与排版 (Scene Counter)](#82-场景页码计数器设计与排版-scene-counter)
  - [8.3 步骤胶囊 (Tab) 超长自适应机制](#83-步骤胶囊-tab-超长自适应机制)
  - [8.4 场景数量 (Scenes) 的理论与实际极限](#84-场景数量-scenes-的理论与实际极限)
...
### 1.2 生成 100% 独立的离线单文件 HTML (build-standalone)
...
## 8. 控制栏配置与海量场景 (Scenes) 扩展与极限

### 8.1 在 config.json 中定制控制栏 (meta.controls)

控制栏的显示状态、自动轮播间隔与调试开关，均可在 `config.json` 的 `meta.controls` 中显式声明：

```json
{
  "meta": {
    "title": "LuxeHMS Architecture Showcase",
    "theme": {
      "bg": "#0a0e17",
      "accent": "#38bdf8"
    },
...
### 8.2 场景页码计数器设计与排版 (Scene Counter)

在播放键 `[▶]` 与步骤胶囊之间，内置了常驻的场景页码计数器（如 `03 / 05`）：

```
                         底部控制栏实时排版
┌────────────────────────────────────────────────────────────────────────┐
│  [▶]   03 / 05   |   [ 1. Overview ]  [ 2. Data & Cache ]  [ 3. Live ] │
└────────────────────────────────────────────────────────────────────────┘
```

#### 核心设计与排版特性：
1. **等宽数字与防抖动（Monospace Typography）**：
   * 采用 `JetBrains Mono` / `SF Mono` 等宽字体与两位数补零格式（如 `01 / 05`）；
   * 场景切换时数字跳动**宽度绝对静止，绝不抖动**，呈现仪表盘般的专业质感；
2. **色彩分层与视觉层级**：
   * **当前页码**：使用主题高亮青蓝色（`#38bdf8`，700 加粗）；
   * **分隔斜杠**：使用低透明度白色（`rgba(255, 255, 255, 0.2)`）；
   * **总页数**：使用暗灰次要色（`#64748b`）；
3. **DSL 声明控制**：
   * 默认自动开启，可在 `config.json` 的 `meta.controls` 中通过 `"showCounter": false` 随时关闭。

---

### 8.3 步骤胶囊 (Tab) 超长自适应机制

在 FocusFlow 中，底部步骤胶囊数量完全由 `scenes` 数组长度决定。为应对多场景下的排版问题，引擎内置了 3 重保护：

1. **横向弹性限宽**：控制栏容器设置了 `max-width: min(92vw, 1100px)`，在任何设备上都不会溢出屏幕边缘；
2. **平滑横向滚动 (Horizontal Scroll)**：Tab 容器内置 `overflow-x: auto` 与隐藏式滚动条，当场景数量较多时支持鼠标滚轮、触摸板或触屏左右平滑滑动；
3. **激活项自动居中对齐 (Auto Scroll-Into-View)**：无论通过键盘（`→` / `Space`）、自动轮播还是点击切换场景，当前激活的步骤胶囊会自动平滑滚动居中，永远处于视线正中央。

---

### 8.4 场景数量 (Scenes) 的理论与实际极限

```
+-----------------------------------------------------------------------------------------+
|                               场景数量 (Scenes) 扩展与极限全景                            |
+--------------------------+--------------------------------------------------------------+
| 🚀 理论与技术极限 (Engine)  | • DOM & 内存：纯数据驱动虚拟状态机，每个场景仅占几行 JSON       |
|                          | • 调度性能：GPU 运镜与 Double-RAF 始终维持 60fps 满帧            |
|                          | • 理论上限：100 ~ 1000+ 个场景无任何技术压力                   |
+--------------------------+--------------------------------------------------------------+
| 🧠 人机工效与最佳体验 (UX) | • 🥇 黄金体验 (3 ~ 7 个场景)：符合米勒定律 (7±2)，最适合演讲汇报   |
|                          | • 🥈 中型故事线 (8 ~ 15 个场景)：适合完整子系统端到端链路演示    |
|                          | • ⚠️ 超过 15+ 场景：建议精简 Tab 标题为短词 (如 GW, Auth, MQ)，   |
|                          |   或拆分为多个专题演示 (如《交易系统篇》、《容灾架构篇》)        |
+--------------------------+--------------------------------------------------------------+
```
