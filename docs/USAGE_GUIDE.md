...
- [5. 高亮选框、流光连线与端点脉冲标定指南](#5-高亮选框流光连线与端点脉冲标定指南)
  - [5.1 粗拉框 + 智能像素贴合 (Smart Snap on Drag · 推荐主力)](#51-粗拉框--智能像素贴合-smart-snap-on-drag--推荐主力)
  - [5.2 Option/Alt+单击单点智能吸附 (Sobel & Multi-Ray Snap)](#52-optionalt单击单点智能吸附-sobel--multi-ray-snap)
...
在标定助手激活状态下（`⌘+Shift+D` / `Ctrl+Shift+D`），可以通过以下方式极速提取卡片与连线配置：

### 5.1 粗拉框 + 智能像素贴合 (Smart Snap on Drag · 推荐主力)

FocusFlow 内置了**边界导向式轮廓精修算法（Auto-Refine）**，创作者**无需眯着眼睛小心翼翼对齐像素**，只需随手粗拉一个框即可：
...
```
       ...
       │                                                        │
       │    ┌ - - - - - - - - - - - - - - - - - - - - - - ┐     │
       │    ┆ 1. 在目标卡片外围随手粗略拉一个框 (0.3 秒) ┆     │
       │    └ - - - - - - - - - - - - - - - - - - - - - - ┘     │
       │                           ⬇                            │
       │           【松开鼠标瞬间：算法在 ±24px 极窄带内微调】  │
       │                           ⬇                            │
       │    ┌─────────────────────────────────────────────┐     │
       │    │ ✨“啪”的一声，瞬间像素级咬合在卡片物理外框! │     │
       │    └─────────────────────────────────────────────┘     │
       │                                                        │
       └────────────────────────────────────────────────────────┘
```

1. **随手粗拉**：按住鼠标左键在目标卡片周围大致拖拽出一个矩形框；
2. **松手自动咬合**：松开鼠标的瞬间，底层 Sobel 窄带算法以 $<0.5\text{ms}$ 极速自动扫描 $\pm 24\text{px}$ 内的梯度最大值，“啪”的一声瞬间自动微调吸附在卡片的真实物理边缘上；
3. **剪贴板自动就绪**：选框就绪的同时，标准 JSON 已经自动格式化写入剪贴板（或点击右下角 **“🔲 复制选框”** 重新获取）：
   ```json
   {
     "id": "box-gateway",
     "type": "rect",
   ...
   ```
4. 直接粘贴回 `config.json` 的 `elements.boxes` 数组即可。

#### 💡 如何强制使用 100% 纯手动拉框（规避算法吸附）？

如果你只想框选卡片内部的局部文字/状态徽章、或者故意保留自定义留白（Padding），不想让算法自动贴合，提供两种自由控制方式：

* **方式 1：辅助键一键直通（推荐 · 最快）**：
  * 按住键盘 **`⌘` 或 `Option` (macOS)** / **`Ctrl` 或 `Alt` (Windows)** 的同时拖拽鼠标拉框；
  * 松开鼠标后，系统提示 `🎯 纯手动选框: [x, y, w×h] (100% 原始坐标)`，算法**绝对不改动一分一毫**！
* **方式 2：HUD 标定面板开关（全局切换）**：
  * 在右下角标定助手面板中，直接取消勾选 **`[✓] 智能像素贴合 (Auto-Refine)`**；
  * 取消勾选后，所有拉框均直接作为纯手动坐标生效。

---

### 5.2 Option/Alt+单击单点智能吸附 (Sobel & Multi-Ray Snap)

对于独立色块分明或白底黑框类型的拓扑图，也可以直接使用单点吸附：

1. 按住键盘 **`Option`** 键（macOS）或 **`Alt`** 键（Windows）；
2. 鼠标在卡片内部空白处**单点一下**；
3. 底层 Sobel 空间一阶梯度积分与 1D 能量投影算法会自动向四周扫描能量峰值，自动生成矩形包围盒并复制 JSON。

> **📖 深度技术文档**：关于边缘吸附算法的数学实现细节、失效根因、多线中位数滤波与 Sobel 梯度积分原理，详见专属算法设计文档：[**`docs/EDGE_SNAPPER_ALGORITHM.md`**](file:///Users/xt/WebstormProjects/focusflow/docs/EDGE_SNAPPER_ALGORITHM.md)。
