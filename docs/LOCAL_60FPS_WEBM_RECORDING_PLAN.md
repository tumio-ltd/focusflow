...
8. [后续待解决问题与优化方向 (Roadmap & Backlog)](#8-后续待解决问题与优化方向-roadmap--backlog)
   ...
   - [8.4 & 8.5 [P2 重要] 播放控制栏全场景规范 (MVP 居中胶囊 / 动态时间按需) 与无痕录制物理隔离方案](#84--85-p2-重要-播放控制栏全场景规范-mvp-居中胶囊--动态时间按需-与无痕录制物理隔离方案-unified-playback-island--recording-clean-isolation-architecture)
...
| **P2 (重要)** | **8.4 & 8.5 (合并)** | **播放控制栏全场景规范 (MVP 居中胶囊 / 动态时间按需) 与无痕录制物理隔离方案** | 🟡 核心体验与设计系统 | 🟠 中等 | **第二阶段 (重点落地)** | **架构与体验深度收敛**。将演播三态显隐与全场景控制栏统一，回归 MVP 居中胶囊形态，实现“手动不显时间/自动按需展开”，并以 Document PiP 零 DOM 物理隔离彻底解决录屏幽灵底栏问题。 |
...
### 8.4 & 8.5 [P2 重要] 播放控制栏全场景规范 (MVP 居中胶囊 / 动态时间按需) 与无痕录制物理隔离方案 (Unified Playback Island & Recording Clean Isolation Architecture)

- **核心背景与问题深拆 (Root Cause & Problem Diagnosis)**：
  1. **录制视频幽灵 Record 烧录现象复盘**：
     - 此前为了解决画中画弹出抢焦问题，临时将录制指示器作为 DOM 节点（`recording-hud-bar`）挂载于标签页视口中（虽带 3 秒自动淡出）；
     - 由于浏览器 `getDisplayMedia` 捕获的是标签页视口的真实合成渲染层（Composited Surface），前 3 秒内可见的 `🔴 REC 00:05 · 完成并导出` 被**逐帧不可逆地光栅化烧录进最终视频的像素画面本身**；
     - 当回放导出的视频时，播放器原生控制条在鼠标滑入时正好遮住底部（造成消失假象），鼠标移开或全屏时视频底层被烧录的 REC 像素赫然显现，造成极其诡异的幽灵显隐现象；
     - **底线结论**：**“无痕出片”必须回归绝对的视口物理隔离（Physical Isolation），被录视口内 0 DOM 节点、0 污染！**
  2. **播放控制栏割裂与手动交互下的多余时间噪音**：
     - 此前将分幕指示胶囊放置在左下角、操作按钮放置在屏幕中央，在大屏与宽屏下视线大幅分散，缺乏统一美感；
     - **时间显示心理模型**：在独立单文件 HTML（Standalone HTML）或用户纯手动步进翻页时，用户是以“看 PPT / 看技术架构图”的心态掌控节奏，**并无时间流逝或自动换幕**。此时展示 `00:00 / 00:06` 倒计时不仅无用，还会给用户带来“视频是否卡死”的疑惑或被催促的焦虑感；
     - **因此，时间刻度必须根据运行状态动态感知：手动不显时间，自动播放按需展开。**

---

#### 1. 核心规范表格矩阵

##### 表格一：控制栏状态矩阵（三态显隐 × 播放运行模式 × 时间刻度决策）

| HUD 显隐模式 | 运行状态 (PlayState) | 控制栏形态与包含元素 | 时间刻度显示规则 | 设计初衷与用户心智 |
| :--- | :--- | :--- | :--- | :--- |
| **`full` (完整态)**<br/>*(底部居中统一 MVP 胶囊)* | **手动单步交互态**<br/>*(Paused / Manual)*<br/>典型：单文件 HTML、用户暂停自控 | `[◀] [▶] [▶] 02/05 · 核心架构` `[👁️]`<br/>- 上下翻页按钮 + 播放按钮<br/>- 分幕索引与场景标题<br/>- HUD 模式切换按钮 | **🔴 严格隐藏时间**<br/>*(无时间文字、无 00:00)* | 用户按自己的节奏逐页看架构，就像看 PPT；此时没有时钟在流动，展示时间只会产生“视频是不是卡住了”的误解与催促焦虑。 |
| **`full` (完整态)**<br/>*(底部居中统一 MVP 胶囊)* | **自动演播态**<br/>*(Playing / Auto-advance)*<br/>典型：点击播放、自动巡航排练 | `[◀] [⏸] [▶] 02/05 · 核心架构` `\|` **`⏱️ 00:03 / 00:06`** `[👁️]`<br/>- 胶囊右侧平滑展开微型时间勋章<br/>- 鼠标悬停可展开整场总时间 | **🟢 实时高精度展示**<br/>*(分幕当前/总长，累积整场)* | 用户需要把控演播节奏、观察图元转场动效与台词的配合进度，倒计时与刻度必不可少。 |
| **`minimal` (微缩态)**<br/>*(右下角磨砂胶囊)* | **手动单步交互态**<br/>*(Paused / Manual)* | `02/05` `[⛶ 展开]` `[👁️]`<br/>- 仅保留极简分幕序号<br/>- 一键展开图标 + 切换纯净态图标 | **🔴 严格隐藏时间** | 最小化遮挡画布，仅让演讲者知道当前翻到了第几幕。 |
| **`minimal` (微缩态)**<br/>*(右下角磨砂胶囊)* | **自动演播态**<br/>*(Playing / Auto-advance)* | `00:03 / 00:06 · 02/05` `[⛶ 展开]` `[👁️]`<br/>- 包含呼吸绿点与时间微步进 | **🟢 显示分幕时间**<br/>*(紧凑数字刻度)* | 极度克制的排练计时器，不影响主画面视线。 |
| **`zen` (沉浸纯净态)**<br/>*(全屏清空无遮挡)* | **任何播放状态**<br/>*(平时静止 100% 满屏净空)* | - 视口内 **0 元素**<br/>- 鼠标移动/划入底部：浮现 `[👁️ 纯净演播中 · 点击恢复 (H)]`<br/>- 右上角：显现 `[👁️ 恢复]` 图标 | **🔘 隐藏于唤醒浮岛中**<br/>*(静止时全隐，唤醒时按当前播放态按需展示)* | 用于正式大屏投影与无干扰演讲，视线 100% 聚焦架构画布；晃动鼠标随时召之即来。 |

---

##### 表格二：三层架构职责边界、依赖流向与生命周期关系

| 架构层级 | 模块名称与定位 | 核心职责 (Core Responsibilities) | 上游输入 (Inputs) 与下游联动 (Outputs) | 生命周期与防呆容错 |
| :--- | :--- | :--- | :--- | :--- |
| **层 1：表现层**<br/>*(Presentation)* | **MVP 统一控制胶囊**<br/>`PlaybackUnifiedIsland` | - 负责单一居中高质感胶囊的 DOM 渲染；<br/>- 根据 `isPlaying` 自适应决定是否挂载时间刻度；<br/>- 承载翻页、启停、HUD 切换基础交互。 | **输入**：`isPlaying`, `sceneIdx`, `elapsedMs`, `hudMode`<br/>**输出**：用户点击事件通知 Player 状态机。 | 纯组件逻辑，随宿主容器自动渲染，无副作用。 |
| **层 2：编排层**<br/>*(Orchestration)* | **演播 HUD 三态调度器**<br/>`PlaybackHudOrchestrator` | - 调度 `full` ⇋ `minimal` ⇋ `zen` 状态流转；<br/>- 监听鼠标活跃度（3 秒无操作平滑渐隐）；<br/>- 铺设底部感应区与右上角唤醒恢复机制；<br/>- 管理 `localStorage` 偏好持久化。 | **输入**：用户键鼠事件（H 键、底部感应、ESC、点击）；<br/>**输出**：决定层 1 的渲染形态（完整/胶囊/唤醒条/销毁）。 | **安全复位保障**：退出演播弹窗时，若处于 `zen`，自动重置为 `full`，彻底杜绝“再次打开找不回控制栏”。 |
| **层 3：隔离层**<br/>*(Isolation)* | **视频录制物理隔离引擎**<br/>`RecordingCleanEngine` | - **对层 1 和层 2 实施绝对物理熔断拦截**；<br/>- 当 `isRecording: true` 时，**视口内直接强制 0 DOM**（绝不渲染任何录制条）；<br/>- 将 REC 时间与【完成导出】完全托付给 OS 原生 **Document PiP 独立画中画**。 | **输入**：录制会话启动信号；<br/>**输出**：驱动桌面 PiP 独立浮窗，向 MediaRecorder 推送纯净标签页光栅化流。 | **兜底容错机制**：若系统不支持 PiP，仅凭自然播完自动导出或 `ESC` 键提前收尾，**严禁向画布视口妥协塞入任何 DOM 节点**。 |

---

##### 表格三：典型业务场景与各层协同全景映射表

| 业务场景 | 宿主载体 | 运行模态 | 层 1 (表现层外观) | 层 2 (编排层行为) | 层 3 (隔离层状态) |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **场景 A：单文件 HTML 独立交付** | 离线 `.html` 文件 | 用户手动翻页为主 | **MVP 居中胶囊 (无时间)**，仅显示分幕名与翻页 | 支持全屏 F、支持 H 隐藏为极简，鼠标静止渐隐 | **休眠** (不涉及视频录制) |
| **场景 B：Studio 演播全屏排练** | Studio 演播大屏 (`AudienceModal`) | 自动连续演播 (`isPlaying=true`) | **MVP 居中胶囊 (展开时间)**，精确秒表倒计时 | 支持三态流转，支持底部感应唤醒，退出自动复位 | **休眠** (用户正在实时演播，未点击录制) |
| **场景 C：团队正式大屏投影** | 投影仪 / 外部大屏 | 演讲者自由讲述 (`zen 模式`) | **100% 满屏无遮挡**，鼠标移动显现微弱唤醒浮岛 | 3 秒静止全隐，右上角常驻脉冲 EyeOff 图标 | **休眠** (纯演播态) |
| **场景 D：导出 60FPS 影视级视频** | 浏览器屏幕捕获标签页 | 自动逐幕录制推演 | **🚫 强制 0 DOM** (层 1 完全不挂载) | **🚫 强制 0 DOM** (层 2 不响应视口浮岛) | **🟢 强力介入**：主视口 100% 纯净无痕，桌面 Document PiP 独占控制 REC 进度 |

---

#### 2. 详细架构设计规范

##### (1) 层 1 表现层：MVP 经典统一居中胶囊规范 (`PlaybackUnifiedIsland`)
- **视觉规格**：
  - 单一浮岛结构：`bottom: 24px, left: 50%, transform: translateX(-50%)`；
  - Linear/Raycast 质感：`rounded-full bg-slate-900/90 backdrop-blur-md border border-slate-800/80 shadow-2xl`，高度 40px；
- **自适应流式排布**：
  1. 控制区：`[◀ 上一幕]` `[▶ 播放/暂停]` `[▶ 下一幕]`；
  2. 垂直微分割线：`h-4 w-px bg-slate-800`；
  3. 场景信息区：`02 / 05 · 分幕标题`（标题文本过长时 `max-w-[200px] truncate`）；
  4. **动态时间微勋章**：
     - 当 `isPlaying: true` 时，通过 CSS transition 平滑展开：`⏱️ 00:03 / 00:06`；
     - 当 `isPlaying: false` 时，收缩并完全隐藏该区域，保持视觉绝对干净；
  5. 垂直微分割线：`h-4 w-px bg-slate-800`；
  6. HUD 切换区：`<Eye className="w-4 h-4 text-cyan-400" />`（提示“切换 HUD 模式: 胶囊/纯净，快捷键 H”）。

##### (2) 层 2 编排层：HUD 三态流转与防丢安全复位 (`PlaybackHudOrchestrator`)
- **三态流转模型**：
  `full (完整居中胶囊) ──[点击Eye/按H]──> minimal (微缩角落胶囊) ──[点击Eye/按H]──> zen (沉浸纯净) ──[点击/按H]──> full`；
- **`minimal` 胶囊优化**：
  - 放置于右下角，高度 32px，包含极简分幕 `02/05`、`<Maximize2>` 展开图标与模式切换图标；
  - 点击胶囊内任意文字或展开图标直接展开为 `full`；
- **`zen` 沉浸纯净态防丢保障**：
  - 屏幕底部铺设 64px 高度的隐形鼠标移动感应区；
  - 鼠标移动或划到底部时，底部正中平滑浮现唤醒药丸：`[👁️ 纯净演播中 · 点击恢复控制栏 (快捷键 H)]`；
  - 右上角全屏/关闭工具栏中常驻高亮脉冲的 `<EyeOff>` 图标，支持鼠标随时一键恢复；
  - **退出自动复位**：当用户按 `ESC` 或点击关闭退出模态框时，若处于 `zen` 模式，系统自动将其复位为 `full`，确保下次打开依然可见控制栏。

##### (3) 层 3 隔离层：视频录制物理隔离纯净引擎 (`RecordingCleanEngine`)
- **视口物理净空底线**：
  - 当 `isRecording: true` 时，`AudienceModal` **坚决不渲染任何录制进度条或 REC 浮岛**，视口 DOM 树保持 100% 纯粹图元舞台；
  - 彻底终结被捕获标签页像素烧录进 REC 浮岛的 Bug，保证出片视频在任何播放器下回放均绝无幽灵底栏；
- **桌面级独立控制台 (Chrome 111+ Document PiP)**：
  - 在物理独立的 OS 顶层窗口（340×130）中呈现：`🔴 REC 00:12 / 01:28` 计时、帧率、进度条与【完成并保存】；
  - 先完成标签页媒体流锁定（`getDisplayMedia`），再拉起画中画，物理上杜绝浮窗夺焦导致录到浮窗本身；
- **环境不支持降级兜底**：
  - 若处于 Safari / Firefox 或用户禁用画中画环境，系统采用静默全幕录制；
  - 全幕演播自然结束后自动停止录制并触发下载，或用户随时按下 `ESC` 键提前收尾导出，视口内绝不妥协注入 DOM。

---

#### 3. 实施 Checklist 与测试用例规划

##### (1) 执行 Checklist
- [ ] **1. 清理录制视口污染（解决幽灵 REC 条）**：
  - 移除 `AudienceModal` DOM 内部的 `recording-hud-bar`；
  - 录制时仅由 Document PiP 承担交互控制，恢复 100% 绝对纯净录制视口；
- [ ] **2. MVP 统一居中控制胶囊重构**：
  - 将左下角分离的 `audience-scene-pill` 与居中的 `audience-controls` 整合为单一的 `PlaybackUnifiedIsland`；
  - 保证高审美质感、水平居中，消除分块割裂感；
- [ ] **3. 动态时间显隐按需控制**：
  - 挂载 `isPlaying` 驱动逻辑：手动单步暂停态下严格隐藏时间；自动播放态下平滑展开时间微勋章；
- [ ] **4. 完善三态无缝流转与防丢机制**：
  - 保持 `minimal` 胶囊与 `zen` 模式；
  - 完善底部感应区唤醒药丸与退出时自动安全复位为 `full`；
- [ ] **5. 下沉通用组件至 `@focusflow/player`**：
  - 为 Standalone HTML 独立单文件打包提供同款 MVP 统一胶囊。

##### (2) 自动化 E2E 测试用例规划 (Playwright)
- [x] **`TC577: 演播播放态 HUD 三态显隐、动态时间与防丢唤醒验证 (test_playback_hud_modes)`**：
  1. 验证默认 `full` 模式下为单一居中 MVP 胶囊；
  2. 验证暂停/手动状态下时间刻度严格不可见；
  3. 触发播放后，验证时间刻度区域平滑展开并递增（`00:00` -> `00:01`）；
  4. 验证 `H` 键及图标切换至 `minimal` 与 `zen` 模式；
  5. 验证在 `zen` 模式下鼠标移动触发唤醒药丸 `zen-restore-hud-btn`，点击一键恢复 `full`；
  6. 验证退出演播弹窗后自动安全复位偏好为 `full`。
- [x] **`TC578: 录制时 100% 纯净视口与独立画中画生命周期验证 (test_recording_pip_isolation)`**：
  1. 启动视频录制，验证主标签页视口 DOM 树中 0 录制控制器节点；
  2. 验证独立 Document PiP 窗口接收时间更新（`REC 00:01`）；
  3. 点击 PiP 窗口内的【完成并保存】按钮，触发录制流 `stop` 并触发 WebM 下载；
  4. 验证出片视频流不含任何 REC 字符或水印污染。
